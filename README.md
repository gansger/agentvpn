...
Mini App и панель администратора никогда не обращаются к 3x-ui или Robokassa напрямую.

Текущее состояние репозитория соответствует **Этапу 4: интеграция платежей Robokassa и СБП**.
...
- Реализован `RobokassaPaymentProvider`, создающий hosted checkout только с методом СБП.
- Добавлен подписанный Robokassa ResultURL с идемпотентной активацией подписки и проверкой
  номера счёта, order ID и суммы платежа.
- Добавлена read-only проверка доступности СБП в магазине Robokassa.
...
- публичный домен;
- токен Telegram-бота;
- MerchantLogin Robokassa и подтверждение наличия активного метода СБП;
...
[инструкции Этапа 4](docs/runbooks/stage-4-robokassa-sbp-payments.md).
